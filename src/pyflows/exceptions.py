...
class PyflowsError(Exception):
    """Base exception for all pyflows errors."""


class WorkflowNotFoundError(PyflowsError):
    def __init__(self, workflow_id: str) -> None:
        super().__init__(f"Workflow not found: {workflow_id}")
        self.workflow_id = workflow_id


class WorkflowAlreadyExistsError(PyflowsError):
    def __init__(self, workflow_id: str) -> None:
        super().__init__(f"Workflow already exists: {workflow_id}")
        self.workflow_id = workflow_id


class StepExecutionError(PyflowsError):
...
class BackendNotInitializedError(PyflowsError):
